...
| `lr_check_schedule.py [--dry-run]` | Cron script — **once daily at 19:30** (changed 2026-09-08 from hourly `:23` per Bahman's request; publishing all due records in one hourly sweep was confusing him — e.g. an immediate-publish request and a delayed makeup post landing in the same tick read as the scheduler ignoring instructions). Publishes any record with `pipeline_state: "scheduled"` whose `scheduled_for` has passed, via the same shared `publish_feed_photo()`. Mirrors kavosh/dariche's `scripts/check_schedule.py` pattern (same alert-on-every-failure, `MAX_ATTEMPTS = 3` reasoning) — **but note the retry has gotten much coarser**: there's no in-process retry, each invocation tries a due record once and leaves it to "the next tick" on failure — with hourly ticks that meant retries roughly an hour apart (3 attempts ≈ 2-3 hours); at once-daily, 3 attempts now span 3 calendar days before the pipeline gives up and needs a human. A story-publish-specific failure is reported as `"auto-retry"` severity via `report_error.py` (see below) — `telegram_receive.py`'s cron scan (still every 5 min) picks it up and retries automatically, independent of this daily cadence. |
